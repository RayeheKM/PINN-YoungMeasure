...
def visualize_model_updated_minibatch(model, loss_history, term1_history, term2_history, term3_history, x, y, z, w):
    ...
    plt.plot(term3_history, label='Term 3', color='orange')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.yscale('log')
    plt.legend()
...
torch.save(trained_model.state_dict(), "model_parameters2D13.pth")

# To load the model parameters from a file
trained_model.load_state_dict(torch.load("model_parameters2D13.pth"))
