...
def loss_function(x, z, F):
    ...
    for i in range(N):
      ...
      term2_vector = dF_dz[i,:] * torch.exp(-(z[0,:]**2) / 2)
      term2Complete += term2_vector.sum()

    term1Complete = term1Complete / N

    term2Complete = weightvalue*(term2Complete/ (N * M) - 1)**2
    loss = term1Complete + term2Complete
    
    return loss, term1Complete.item(), term2Complete.item()
...
def train_pinn_updated(load_pretrained=False, pretrained_path="model_parameters.pth"):
    ...
    for epoch in range(epochs):
        ...
        # Compute the loss
        loss, term1, term2 = loss_function(X, Z, F)
    ...
    return model, loss_history, term1_history, term2_history, X, Z

# Updated Visualization function with adjusted vertical axis labels and further shifted plots
def visualize_model_updated(model, loss_history, term1_history, term2_history, x, z):

    # Plotting the loss over epochs with logarithmic scale

    plt.figure(figsize=(6, 3))
    plt.plot(loss_history, label='Total Loss', color='blue')
    plt.plot(term1_history, label='Term 1', color='red')
    plt.plot(term2_history, label='Term 2', color='green')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.yscale('log')
    plt.legend()
    ...
    plt.savefig('1D2_loss_over_epochs_log.png')
    plt.close()
    ...
    fig_3d_surface = plt.figure(figsize=(2.5, 2))
    ...
    ax_3d_surface.set_ylabel('z')
    ax_3d_surface.set_zlabel('F(x, z)')
    ax_3d_surface.view_init(elev=30, azim=30)
    fig_3d_surface.subplots_adjust(left=0.2, right=0.8, top=0.8, bottom=0.2)
    plt.savefig('1D2_3d_surface_F.png')
    plt.close()

    # Plotting dF/dz as a 3D surface
    fig_dFdz = plt.figure(figsize=(2.5, 2))
    ...
    ax_dFdz.set_xlabel('x')
    ax_dFdz.set_ylabel('z')
    ax_dFdz.set_zlabel('dF/dz')
    ax_dFdz.set_zlim(-1.5, 1.5)
    ax_dFdz.view_init(elev=30, azim=30)
    fig_dFdz.subplots_adjust(left=0.2, right=0.8, top=0.8, bottom=0.2)
    plt.savefig('1D2_3d_surface_dFdz.png')
    plt.close()
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig('1d2_plot_F_vs_z.png')
    plt.close()
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig('1d2_plot_dFdz_vs_z.png')
    plt.close()
    ...
    plt.hist(dF_dz_values, bins=40, density=True, alpha=0.6, color='r')
    plt.xlabel('dF/dz values')
    plt.ylabel('Density')
    #plt.grid()
    #plt.xlim([-1.5, 1.5])
    plt.tight_layout()
    plt.savefig('1D2_histogram_dFdz.png')
    plt.close()
    ...
    plt.plot(X, U, label='U(x_n)', color='blue')
    plt.xlabel('x')
    plt.ylabel('U')
    plt.grid()
    plt.tight_layout()
    plt.savefig('1D2_U.png')  # Save the figure
    plt.close()
...
trained_model, loss_history, term1_history, term2_history, x, z = train_pinn_updated()

# Visualize the results
visualize_model_updated(trained_model, loss_history, term1_history, term2_history, x.to('cuda'), z.to('cuda'))
...
torch.save(trained_model.state_dict(), "model_parameters1D2.pth")

# To load the model parameters from a file
trained_model.load_state_dict(torch.load("model_parameters1D2.pth"))
